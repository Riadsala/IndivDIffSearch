...
{
	filename = paste(datFolder, pp, '/lineseg/Ul', pp, '1', '.dat', sep="")
	pDat1 = read.csv(filename, sep="\t")
	pDat1$session = 1
	filename = paste(datFolder, pp, '/lineseg/Ul', pp, '2', '.dat', sep="")
	pDat2 = read.csv(filename, sep="\t")
	pDat2$session = 2
	pDat = rbind(pDat1, pDat2)
	pDat$subj = pp
	dat = rbind(dat, pDat)
	# remove undeeded things
	rm(filename, pDat1, pDat2, pDat)
}

names(dat) = c("trialNum", "targPresent", "targSide", "filename", "key", "rt", "message", "trialType", "hemianopia","session", "subj")
