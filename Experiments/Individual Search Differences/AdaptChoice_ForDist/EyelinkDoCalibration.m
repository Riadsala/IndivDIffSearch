function result=EyelinkDoCalibration(el, sendkey)

% EDITED VERSION OF EyelinkDoTrackerSetup
% AUTOMATICALLY SKIPS STRAIGHT TO CALIBRATING WITHOUT GOING TO SETUP SCREEN
% USES EDITED FILE EyelinkTargetModeDisplay2 (INSTEAD OF EyelinkTargetModeDisplay)
% MADE BY JESS IRONS MARCH 2015
...
while stop==0 && bitand(Eyelink( 'CurrentMode'), el.IN_SETUP_MODE)
	...
	if bitand(i, el.IN_TARGET_MODE)			% calibrate, validate, etc: show targets
        ...
        %%% DO THE ALTERED VERSION OF TARGETMODEDISPLAY
		EyelinkTargetModeDisplay2(el);	
        %%%% EXIT STRAIGHT AFTER CALIBRATION
%         [resultfinal, ~] = Eyelink('CalMessage');
%         if resultfinal == 21
            stop = 1;
%         end
